...
def main():
    ...
    if is_4K:
        main_lines[9] = re.sub(r"\[Latest: [0-9]+\.[0-9]+(?:[^\]]*?)<br/>\((202\d-\d\d-\d\d)\)]\({}\)".format(args.target), "[Latest: {}<br/>({})]({})".format(version,upload_date,args.target), main_lines[9])
    else:
        main_lines[10] = re.sub(r"\[Latest: [0-9]+\.[0-9]+(?:[^\]]*?)<br/>\((202\d-\d\d-\d\d)\)]\({}\)".format(args.target), "[Latest: {}<br/>({})]({})".format(version,upload_date,args.target), main_lines[10])
